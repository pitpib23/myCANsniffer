...
_LIVE_INTERFACES = [
    ...
    "ixxat", "neovi", "systec", "usb2can", "udp_multicast",
]
...
class ConfigDialog(ResponsiveDialog):
    ...
    def _build_source_tab(self) -> QWidget:
        ...
        # -- live
        self.live_box = QGroupBox("Live capture (listen-only)")
        live_form = QFormLayout(self.live_box)

        self.live_interface = QComboBox()
        self.live_interface.setEditable(True)
        self.live_interface.addItems(_LIVE_INTERFACES)
        self.live_interface.setCurrentText(str(self.config.get("source.live.interface", "virtual")))
        self.live_interface.currentTextChanged.connect(self._on_interface_changed)
        live_form.addRow("Interface", self.live_interface)

        self.live_channel = QLineEdit(str(self.config.get("source.live.channel", "0")))
        live_form.addRow("Channel", self.live_channel)
    ...
    def _on_accept(self) -> None:
        ...
        data["source"]["live"] = {
            "interface": self.live_interface.currentText().strip().lower(),
            "channel": self.live_channel.text().strip(),
