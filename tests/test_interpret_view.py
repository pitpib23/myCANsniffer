...
@unittest.skipUnless(HAVE_QT, "PySide6 not available")
class ScaledValueUpdateTests(unittest.TestCase):
    """Editing a signal rule must reach the Scaled value column.

    Regression: the render memo fingerprinted the rules by *count*, so adding
    or deleting one rebuilt the table but editing one in place did not — the
    column kept showing values computed from the previous scale and offset.
    """
    ...
    def setUp(self):
        self.config = Config.defaults(
            os.path.join(os.environ.get("TEMP", "."), "cansniff_scaled_test.json")
        )
        self.config.set("interpret.word_size", 2)
        self.config.data["signals"] = [{
            "name": "Speed", "enabled": True, "can_id": "0x101", "channel": "",
            "offset": 0, "length": 2, "decoder": "u16_be",
            "scale": 1.0, "add": 0.0, "unit": "", "precision": 1,
        }]
        self.view = InterpretView(self.config, Theme())
        self.view.show_frame(_frame(bytes.fromhex("01F4000000000000")))
        self.app.processEvents()
    ...
    def _scaled_column(self):
        column = self.view.table.columnCount() - 1
        return [(self.view.table.item(r, column).text()
                 if self.view.table.item(r, column) else "")
                for r in range(self.view.table.rowCount())]

    def test_baseline_shows_the_unscaled_value(self):
        self.assertIn("Speed = 500.0", self._scaled_column())

    def test_editing_scale_updates_without_forcing(self):
        # Exactly what the Signals dialog does on OK: replace the list, then
        # ask the panel to refresh. No force= anywhere.
        self.config.data["signals"][0]["scale"] = 0.1
        self.view.refresh()
        self.app.processEvents()
        self.assertIn("Speed = 50.0", self._scaled_column())

    def test_editing_offset_unit_and_precision_updates(self):
        rule = self.config.data["signals"][0]
        rule.update({"scale": 0.1, "add": -40.0, "unit": "kph", "precision": 2})
        self.view.refresh()
        self.app.processEvents()
        self.assertIn("Speed = 10.00 kph", self._scaled_column())

    def test_renaming_a_rule_updates(self):
        self.config.data["signals"][0]["name"] = "Wheel"
        self.view.refresh()
        self.app.processEvents()
        self.assertIn("Wheel = 500.0", self._scaled_column())

    def test_disabling_a_rule_clears_the_column(self):
        self.config.data["signals"][0]["enabled"] = False
        self.view.refresh()
        self.app.processEvents()
        self.assertEqual([v for v in self._scaled_column() if v], [])

    def test_count_alone_does_not_decide(self):
        """Swapping one rule for a different one keeps the count identical."""
        before = self.view._render_key
        self.config.data["signals"][0]["decoder"] = "u16_le"
        self.view.refresh()
        self.assertNotEqual(self.view._render_key, before)
        self.assertIn("Speed = 62465.0", self._scaled_column())

    def test_unrelated_refresh_still_skips_the_rebuild(self):
        """The fix must not defeat the memo it lives in."""
        calls = []
        original = self.view._fill_table
        self.view._fill_table = lambda result: calls.append(result)
        self.view.refresh()
        self.view.refresh()
        self.view._fill_table = original
        self.assertEqual(calls, [], "unchanged rules should not rebuild")

    def test_a_hand_edited_config_cannot_break_the_panel(self):
        self.config.data["signals"] = ["nonsense", {"scale": [1, 2]}]
        self.view.refresh()          # must not raise
        self.app.processEvents()
        self.assertEqual([v for v in self._scaled_column() if v], [])
